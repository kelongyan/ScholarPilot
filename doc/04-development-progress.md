...
| User auth and RBAC | In progress | Minimal Bearer token/JWT auth, role checks, KB-scope filtering, and frontend role-gated controls implemented |
...
1. Run an end-to-end auth-enabled smoke test with `AUTH_ENABLED=true`, one admin token, one KB manager token, and one user token.
2. Run an end-to-end evaluation against an indexed knowledge base and verify the Evaluations and Audit Logs panels show the run with the configured actor id.
3. Add real user/password login and persisted users only when local token/JWT boundaries are validated.
